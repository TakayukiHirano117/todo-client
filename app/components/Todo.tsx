...
const Todo = ({ todo }: TodoProps) => {
  ...
  const toddleTodoCompletion = async(id: number, isCompleted: boolean) => {
    const response = await fetch(`${API_URL}/editTodo/${todo.id}`, {
      method: "PUT",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({
          ...
          <div className="flex items-center">
            <input
              ...
              className="h-4 w-4 text-teal-600 focus:ring-teal-500
                    ...
                    onChange={() => {
                      toddleTodoCompletion(todo.id, todo.isCompleted)
                    }}
            />
            <label className="ml-3 block text-gray-900">
              {isEditing ? <input type="text" className='border rounded py-1 px-2' value={editedTitle} onChange={(e) => {
